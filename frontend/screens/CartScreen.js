...
const CartScreen = {
  after_render: () => {
    ...
    Array.from(qtySelects).forEach((qtySelect) => qtySelect.addEventListener('change', (e) => {
  ...
  [...Array(item.countInStock).keys()].map((x) => (item.qty === x + 1
    ...
    <div class="cart-action">
      <h3>Subtotal (${cartItems.reduce((sum, item) => sum += item.qty, 0)} items): $${cartItems.reduce((totalPrice, item) => totalPrice + item.qty * item.price, 0)} 
      </h3>
